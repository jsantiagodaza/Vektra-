...
public class MapaView extends JPanel {
    ...
    private void initFX(){
     ...
     private String obtenerRutasJson() {
         ...
         try (Connection con = Conexion.conectar();
              Statement stmt = con.createStatement();
              ResultSet rs = stmt.executeQuery("SELECT id, estacion_origen_id, estacion_destino_id, tiempo_minutos FROM rutas")) {
             boolean first = true;
             while (rs.next()) {
                 if (!first) {
                     json.append(",");
                 ...
                 json.append("{")
                     ...
                     .append("\"tiempo\":").append(rs.getDouble("tiempo_minutos"))
                     .append("}");
             }
         } catch (Exception e) {
